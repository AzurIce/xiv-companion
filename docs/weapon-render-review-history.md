...
- character normal-channel alpha 已删除无证据的 vertex-A 乘法：MeddleTools `meddle character.shpk` 只把 normal Blue 接到 Alpha，tattoo 只把 normal Alpha 接到 Alpha，两者均无 vertex color A 输入。45050 `_b.mtrl` 为 `Blend + GetValuesMultiMaterial`，其毛发 mesh 有 445/807 个顶点 A=0、400/848 个三角形全 A=0；旧实现会直接抹掉这些几何。WGSL 现对 NormalBlue/NormalAlpha 使用中性 vertex alpha，generic BaseColorAlpha、crest 和 lightshaft 保持原行为。synthetic WGPU 固定 vertex A=0/1 输出相同并验证 normal B/A 仍生效；45050 final 前景由 29,559 增至 33,358，alpha debug 保留毛束边缘/缝隙透明。`GetValuesMultiMaterial` 的准确分区公式仍等待额外证据。
