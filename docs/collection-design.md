...
`ItemAction.Type == 2633` 是通用解锁动作，不等同于情感动作。生成时按中文物品语义拆分为情感动作、发型与面妆、详细地图、青魔法、方城声援、肖像教材、建造许可、成就解锁及其他解锁，避免把所有解锁型物品混入情感动作。

其他永久解锁动作还包括肖像教材旧类型、九宫幻卡、陆行鸟装甲、制作秘籍、战果/调查记录、面部配饰、新月岛辅助职业及成就证书。九宫幻卡、鸟甲、面部配饰、生产秘籍使用独立分类；战果/调查记录和辅助职业收入“其他解锁”，证书收入“成就解锁”。装备箱、随机卡包、临时状态道具等一次性消耗效果不属于收藏图鉴。
