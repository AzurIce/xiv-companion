...
4. Material params：alpha/glass/normal/tile/toon/detail/color/outline/scroll/lightshaft 参数与 `CategoryFlowMapType` 已结构化；water transparency/deep/refraction/whitecap colors 和 wave sampler roles 也已加入。共享 tile/detail atlas、bguvscroll、Flow tangent 与 water alpha/base/primary normal 已有 WGSL 消费。后续处理独立 multi/Map1、water refraction/whitecap、ORB/detail 权重和其它 shader-family 行为。
...
8. 特殊 shader：lightshaft、transparency/glass、bguvscroll、Flow 与 water 已有第一版消费；reflection、stockings、tattoo、occlusion 等仍主要停留在 package 分类，后续补各 family 的实际 shader 行为。
