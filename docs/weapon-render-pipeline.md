...
`g_OutlineColor`、`g_OutlineWidth`、`g_SpecularColorMask`、`g_SSAOMask`、`g_TextureMipBias`、`g_ShadowPosOffset` 已解析进材质数据、phantom summary 和 renderer uniform；WGSL 当前用 `g_OutlineColor/g_OutlineWidth` 做受限 silhouette rim fallback，用 `g_SpecularColorMask` 调制高光颜色/强度，用 `g_SSAOMask` 保守调制环境底光。Meddle 只确认 outline 参数适用于 character/hair/iris/skin 系列且默认宽度为 0，MeddleTools 没有 outline node/bake 行为；当前 24 组 phantom summary 的 outline width 也全部为 0，没有真实正宽度武器样本。下一批会用明确标注为近似的 inverted-hull geometry pass 替换 fragment rim：仅 character family 且 prepared outline flag 为真时，沿 object-space normal 扩张、剔除 front faces、只绘制 outline color；宽度换算先直接使用模型单位并做上限保护，通过 synthetic override 验证。texture LOD 和 shadow offset 仍未实现。
