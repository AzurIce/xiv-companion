...
`g_TileIndex`、`g_TileAlpha`、`g_TileScale` 已解析进材质数据和 renderer uniform；WGSL 已绑定 tile pair atlas，按 ColorTable tile properties 或 `g_TileIndex` 选层，并结合 TileMatrix/TileScale 采样。`TileAlpha` 只控制 tile normal/ORB 权重，不作为材质透明度。
...
`g_DetailID`、`g_MultiDetailID`、`g_DetailColor`、`g_MultiDetailColor`、`g_DetailColorUvScale`、`g_DetailNormalUvScale` 已解析进材质数据和 renderer uniform；WGSL 已绑定 detail pair atlas，按两个 ID 选层并分别采样 diffuse/normal。当前缺少真实 bg 武器样本，混合权重仍待校准。
...
`g_UVScrollTime` / `0x9A696A17` 已按 MeddleTools `UvScrollMapping` 转换成 UV0/UV1 scroll multiplier 并进入 renderer uniform。`bguvscroll.shpk` 已单独分类，prepared per-role mask 当前只让已绑定的 primary base/normal/specular 使用 UV0Scroll；mask、index、ColorTable、tile/detail 等保持静态，`characterscroll.shpk` 不会因 family 名称自动滚动。MeddleTools 证明 Map1 应使用 UV1Scroll，但本仓尚未独立绑定 Map1。Web RAF 驱动和 native snapshot 时间 0 的稳定行为保持不变。
...
2. GPU 顶点格式：uv1-uv3、color1、secondary normal/bitangent、flow 已进入 GPU 顶点输入；PreparedMaterial 已有 feature flags、UV source、per-role scroll mask、flow mode 和 unsupported/runtime-only 输入摘要。`usesFlow` 已收紧为 Flow mode + flow0，WGSL 已消费 primary flow tangent；下一步是 Map1/UV1Scroll、flow1、secondary tangent 与 color1。
...
4. Material params：`g_AlphaThreshold`、`g_AlphaAperture`、`g_AlphaOffset`、`g_ShadowAlphaThreshold`、`g_Transparency`、`g_GlassIOR`、`g_GlassThicknessMax`、normal/tile/toon/detail/color/outline 参数、`g_UVScrollTime` 以及 lightshaft 参数已进入结构化材质字段；`CategoryFlowMapType` 也已结构化。共享 tile/detail atlas、bguvscroll primary role 与 Flow tangent 已有 WGSL 消费。后续要继续处理 water transparency、独立 multi/Map1、ORB/detail 权重和其它 shader-family 行为。
...
8. 特殊 shader：lightshaft、transparency/glass、bguvscroll primary role 和 Flow tangent 已有第一版消费；reflection、stockings、tattoo、occlusion 等仍主要停留在 package 分类，后续补各 family 的实际 shader 行为。
