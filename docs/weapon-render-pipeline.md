...
4. Material params：`g_AlphaThreshold`、`g_NormalScale`、`g_MultiNormalScale`、`g_DetailNormalScale` 与 `g_MultiDetailNormalScale` 已进入结构化材质字段；后续要继续接入 tile/detail/scroll 其他参数，并让 multi/detail normal 实际参与 shader。
