...
- mesh category 会映射成 `ModelMeshDrawRole`，并和 submesh attribute metadata 一起进入第一版 `PreparedModel` / `PreparedMesh`，作为 renderer-friendly 的第一步 prepared draw role。模型含 attribute submesh 时 prepared 阶段按 submesh 所需 attribute mask 计算 visibility；未显式提供 `PreparedModelOptions.enabledAttributeMask` 按游戏默认（全部未启用，= 0）处理，带 attribute 的 submesh 默认隐藏。
- ignored phantom snapshot 的 `model-summary.json` 会输出 mesh category、submesh attributes、bone table、shape 影响摘要、shape target/delta count，并链接 full MDL metadata JSON。MDL `BaseIndicesIndex` 按 mesh-relative index-buffer position 解释；submesh remap 对不同 target signature 的索引出现位置拆点。`ModelRenderer::new_with_prepared_options` 可用显式 `enabledShapeMask` 累加 active target delta；默认 renderer 仍使用 base geometry。native snapshot options 同样可传 preparation options，42697 base/bit0 回归验证了真实 GPU 输出差异。Web 渲染面板会为含 shape 的模型显示 Base/shape-name 选择，画布常驻挂载，shape/物品切换经 `set_model` 在常驻渲染 context 上同步重建对应 GPU buffers，不重复初始化 WebGPU。静态 MDL 不含 runtime shape name/bit mapping，离线 table-order bit 约定不代表游戏默认状态。
...
1. Prepared draw role / pass：`PreparedModel` / `PreparedMesh` 已完成第一步主 pass 过滤并保留 submesh attribute metadata；attribute submesh 默认按游戏语义全部未启用（隐藏，Meddle 默认 EnabledAttributeMask=0），显式 `enabledAttributeMask` 输入可显示对应部件，显式 `enabledShapeMask` 已可在 renderer creation 时应用 sparse morph target，默认离线模式仍使用 base geometry；renderer 内部已有 `Opaque/Cutout/Transparent/Glass/AdditiveLightShaft` prepared pass 分类，Cutout/Glass 已有独立 wgpu pipeline 入口，`AdditiveLightShaft` 已对齐 MeddleTools 当前连接的 Sampler0/Sampler1、vertex B、`g_Color`、emission strength 和 alpha；后续还需要 runtime shape name/bit mapping、更完整的 cutout/glass shader 行为，以及 lightshaft `Type/AngleClip/NearClip` 等游戏语义。
...
## 11. 模型域扩展：装备 / 家具庭具 / 宠物坐骑

渲染管线本身是模型域无关的（材质 family 驱动）；“模型预览”页（`#/equipment-models`，`#/weapon-models` 为别名）按域扩展了路径解析与目录资产。路径语义以 xivModdingFramework（`Mdl.cs`/`Mtrl.cs`/`Housing.cs`/`XivModelChara.cs`）为准。

- **装备（防具/饰品）**：`Item.Model{Main}` raw u64 四段中 quad[2]==0 为装备——quad[0]=套装 id、quad[1]=版本（≈材质集，IMC 未解析）；路径 `chara/equipment/e{set:04}/model/c{race:04}e{set:04}_{slot}.mdl`，饰品在 `chara/accessory/a{set:04}/...`。槽位后缀 3=met、4=top、5=glv、7=dwn、8=sho、9=ear、10=nek、11=wrs、12=rir；腰带(6)无模型，复合部位（15/16/18/20-23）需多模型加载、暂不支持。race 请求值优先、c0101 兜底。染色与武器共用 STM 管线。物品源直接取 collection-catalog。
- **家具/庭具**：物品经 `HousingFurniture`/`HousingYardObject` EXD 的 ModelKey 映射（目录资产 `furniture-catalog.json` 由 datamining CSV + craft-data 生成，无需游戏安装）。模型是 SGB 而非直接 MDL：`bgcommon/hou/{indoor|outdoor}/general/{key:04}/asset/{fun|gar}_b0_m{key:04}.sgb`；数据层按上游算法扫描 SGB 字符串段提取 .mdl/.sgb/.mtrl 引用并递归，多 MDL 合并渲染（component 按 mdl 序号，平铺展示，不恢复原始摆放变换）。材质优先按 SGB 引用列表匹配，缺失回退目录推导。家具无染色/种族维度。**bgcolorchange/bgcrestchange 漫反射纹理 alpha 是染色遮罩而非透明度**（遮罩=0 的非染色部件照常不透明），`has_alpha` 回退不得据此把材质判为 Blend（6601 实证：桌面底座/镶边曾被穿透成洞）。
- **宠物/坐骑**：解锁物品经 `ItemAction`（宠物 Type=853、坐骑 Type=1322，物品经 `Item.ItemAction` 列链接——ItemAction 行 key 不是物品 id）→ `Companion`/`Mount` 第 8 列 → `ModelChara`（Type 2=demihuman、3=monster，其余跳过）。路径：monster `chara/monster/m{model:04}/obj/body/b{base:04}/model/m{model:04}b{base:04}.mdl`；demihuman `chara/demihuman/d{model:04}/obj/equipment/e{base:04}/model/d{model:04}e{base:04}_{slot}.mdl`（slot 候选 met/top/glv/dwn/sho 逐个探测合并，覆盖陆行鸟马具）。材质走通用分支（无 monster/demihuman 特例），版本按 [variant, base, 1, 101, 201] 回退。目录资产 `chara-catalog.json` 同理由 datamining CSV 生成。无染色/种族维度。**MDL 属性部件（attribute submesh，如 atr_bv_a/atr_lod）默认全部未启用**（Meddle 默认 EnabledAttributeMask=0），预览须默认隐藏、仅显式 enabled_attribute_mask 时显示——否则变体部件与本体重叠（自走人偶001号实证）。
- **蒙皮**：装备/宠物/坐骑/角色均为 skinned 模型，MDL 顶点是 bind pose。蒙皮管线已落地（见 §11.3）：rest pose 关节矩阵为恒等，identity 蒙皮输出与非蒙皮路径在浮点噪声级一致；姿势覆盖（`SkeletonPose`）可驱动骨骼，动画（PAP 采样播放）未实现。

三个目录资产的 schema、ResourceHub 注册与 IndexedDB 缓存臂对齐既有 collection-catalog 模式；`EquipmentModelData`/`FurnitureModelData`/`CharaModelData` 均为 `WeaponModelData` 的类型别名，字段语义在各自别名文档注明。

### 11.1 角色拼装（chara/human 玩家角色）

数据层（A 阶段）已落地完整角色拼装链路与捏脸数据资产，渲染页接 D 阶段。

- **捏脸 26 字节**（`CharacterCustomize`，serde camelCase，往返 `to_bytes`/`from_bytes`）：布局以 Anamnesis `ActorCustomizeMemory` 为准；Race byte 1 中原、2 精灵、3 拉拉菲尔、4 猫魅、5 鲁加、6 敖龙、7 硌狮、8 维埃拉（**与 c 编码序不同**：拉拉 c11/12、猫魅 c07/08），Tribe byte 1-16。c 编码 `(pair*2-1+gender)*100+1`，pair 表见 `race_code_from_parts`。
- **部件路径**（真实 SqPack 全量普查/边界测量，2026-09-12 修正"裸模 b 文件"误设）：游戏裸装身体 = **小衣装备 e0001**（`chara/equipment/e0001/model/c{race:04}e0001_{slot}.mdl`；top/dwn = 皮肤躯干/四肢网格 + 内衣网格，全族恒有；sho 仅中原男/女、鲁加男、拉拉男）+ **裸肤 e0000**（glv 裸手恒有；无 e0001 sho 的族以 sho 补裸足。e0000 全槽仅中原男/女/拉拉男有文件，其余族按版型回退：拉拉女→拉拉男、其余→中原同性别，见 `bare_limb_model_race_candidates`）。**硌狮/维埃拉无自身 e0001 文件**，按种族骨变形树回退（硌狮→鲁加、维埃拉→猫魅，`smallclothes_model_race_candidates`；骨变形未应用，比例为回退种族）。`chara/human/.../body/b####` 的 b 文件不是玩家裸模（b0002 实测仅为颈部补丁小件）。皮肤材质恒 `mt_c{肤族}b0001_a`：肤族表经 e0001 内嵌材质名逐族核对（精灵/猫魅→中原同性别、鲁加女→高地女、拉拉女→拉拉男，其余含硌狮/维埃拉自用，`skin_race_for`），跨族回退时游戏仍用角色自身肤族材质（候选根自身肤族优先）。脸 `f{head字节+基}_fac.mdl`（字节 0 基；基=1，唯**硌狮女脸文件仅 f0005-f0008、基=5**；**高地无脸模，借中原同性别**）；发 `h{hair字节}_hir.mdl`（1 基直连）；尾 `t{字节+1}`（猫魅/敖龙）、兔耳 `z{字节+1}`（维埃拉）。
- **发型选项（CharaMakeCustomize 真实结构，已对 18 族真实文件集合全量验证）**：CSV 数据列实为 `key,FeatureID,Data,...`（表头写的 Icon 列不存在）；`FeatureID` = 发型文件 id（h#### 的 ####），`Data = prefix*1000 + 组内 1 基序号`。18 槽，槽 i 占 keys `[1 + 130*i, +129]`，**槽序即 c 编码序 0101..1801**（中原/高地各占 2 槽）；发型 Data prefix = 130 + Race byte（131..=138），面妆 prefix 250/251；**选项顺序 = 槽内 Data 序号升序**（槽尾部存在发型/面妆行交错，不能按行序切，槽 15/16 的尾部即交错实例）。CharaMakeType 的 InitVal 是 0 基选项索引（选项 i = 槽内 Data 序第 i 项；中原男 InitVal=51 → h0148）；SubMenuParam 发型菜单值不是 Data 也不是 FeatureID（≥1000 的值全在 UI 图标段，如 2415/3965）。槽后部的 prefix 130/139 大块（fid 1..29 ×24/×8 组）是特征图标表，资产不提取。xtask 按槽提取进 `character-make.json` 的 `hairOptions`/`facePaintOptions`。
- **材质特例**（`Mtrl.GetMtrlPath`/`GetHairMaterialRoot` + 实测）：皮肤恒 v0001、肤族按 `skin_race_for` 解析（自身肤族根优先，内嵌文件名根消化共享）；脸/兔耳无版本目录；头发共享表（硌狮永不共享；h0101-h0115 五族独占；h0116-h0200 重定向中原；h0201+ 独占）作用于**材质根与文件名**，且共享段模型根分布不规则（h0116 仅中原男根），加载方按 [自身, 共享表根, 中原男, 中原女] 候选回退，材质共享按实际命中的模型路径种族解析。
- **attribute 显隐**（2026-09-12 修正数值掩码误设）：submesh 的 attribute 位是**各 MDL 本地 attribute 表序**（名随掩码置位升序 zip 对应），跨 MDL 数值不可比（c0101 脸 bit0=atr_mim、c1401 脸 bit0=atr_kao、c0801 身 bit0=atr_cn_neck），合并装配后全局数值掩码无意义——此前全局 `0x1` 导致非中原男的身体/头发主形被隐藏。角色显隐按**名**启用（`character_enabled_attribute_names` → `PreparedModelOptions::enabled_attribute_names`）：装备遮罩类（atr_nek/hij/ude/sne/hiz/cn_neck 等）与脸基件（kao/mim/hrn）、发件（kam/bak/sta/top）裸装恒开；**atr_lod 恒开**——名字有误导性，并非"远景 LOD"：敖龙 h0001 的刘海几何、尾巴的中段件都在 atr_lod 子网格里（此前恒关导致秃刘海/断尾实证）；`atr_fv_*` 脸部特征件按 FacialFeatures 字节逐位开关（bit k ↔ fv_{a+k}）。武器/宠物等数值掩码路径（游戏默认全关）不变。
- **component**：`CharacterAssemblyData = WeaponModelData` 别名，`model_sub=None`。现有 component 逻辑对多 MDL 模型按 MDL 根给 component 序号（demihuman 坐骑测试断言 [0,0,1,1]），角色拼装渲染以 `PreparedModelOptions::component_preview_layout=false` 关闭平铺（全零偏移、部件原位重叠），默认 true 保持武器/家具预览行为。
- **捏脸资产**：`character-make.json`（32 组菜单/默认值/特征图标/发型与面妆选项列表，CharaMakeType.csv + CharaMakeCustomize.csv 生成）与 `character-palette.json`（human.cmp：公共区 眼192@块0/挑染208@块1/唇与面妆@块13上下半各96+alpha、种族特征=眼色板；32 组 肤色192/发色192(硌狮)或208；RGBA u8 非 squared；文件尾部 4480 字节为 32×140 字节 RGSP 缩放记录，未解析）。xtask 子命令 `--character-make`/`--character-palette`。
- **加载入口**：`load_character_assembly_from_resource`（+ async + `_from_game_dir`），小衣 top/dwn、裸肤 glv、脸、发必需，sho/尾/兔耳可选探测；`CharacterAssemblyLoadRequest::appearance` 提供 `CharacterAppearanceColors` 时按 shader family 写入部件材质（`ModelMaterial::character_colors`：skin 全收、hair 收发/挑染、iris 收眼色、charactertattoo 收特征色；装备 character 等不消费，uniform 尾部角色色通道全零，武器渲染不受影响）。真实数据验证 `tests/native_character_assembly.rs`（32 组默认装配全过）。

### 11.2 角色材质渲染（肤色/发色/眼色/特征色/面妆）

渲染语义以 MeddleTools 节点配置（`node_configs.py` skin/hair/iris/charactertattoo）+ 游戏 `CustomizeParameter` cbuffer（FFXIVClientStructs/Meddle 实地逆向）为准：

- **颜色解析**（`appearance_colors_from_palette`，纯函数）：customize 字节 → `character-palette.json` 色项（肤色取 (部族,性别) 组 skin[skintone]、发色 tribes.hair[hair_tone]、挑染 common.highlight、眼色 common.eye[l/r]、唇色 common.lip（带 alpha）、特征色 common.feature、面妆色 common.face_paint）。lipstick=mouth bit7、highlights=highlight_type bit7、面妆 reversed=face_paint bit7（FFXIVClientStructs `CustomizeData` 0x18）。RGB 按显示值 u8/255 直接作线性乘子：cbuf 虽以 squared RGB 存储，但预览的稳定预览光照与游戏光照不同，squared 在本管线双重变暗，实测对照安装版默认角色（维埃拉女）显示值复刻游戏观感（差异在 `appearance_color` 注释记录）。角膜环强度无离线来源按 1.0。
- **uniform 布局**：`MaterialUniform` 尾部追加 10 个 vec4（`character_skin/lip/main/mesh/left_iris/right_iris/option/decal/decal_uv/params`），RGB=颜色、W=激活/强度；`character_params.x` 标记 hair family 是否用 mask R 通道作明暗细节。无 `character_colors` 的材质全零，WGSL 分支按激活标记隔离，39 合成快照回归零差异。
- **WGSL 分支**（`resolve_character_family_base`）：skin family  diffuse×SkinColor（Face/Body/BodyJJM 同路径），LipColor 按 lipstick 开关与脸漫反射 alpha（唇形遮罩近似，仅脸材质填通道）混入，面妆 decal 采样叠乘；hair family diffuse×MainColor，无 base 纹理时以 **mask R 通道**（发丝明暗渐变 AO，真实纹理解码验证）作明暗细节——脸部 hair 材质（眉毛等）的 mask 是数据通道，由 `character_params.x` 关闭取发色平色；MeshColor 挑染区域用 **mask G 通道**（R=渐变、G=挑染区域、B≈常量、A=R 副本）；iris family 虹膜×LeftIrisColor（单眼材质双眼共享近似；g_WhiteEyeColor 无离线来源按恒等白）；charactertattoo family ×OptionColor（形状由法线 alpha 的透明策略给）。**hair.shpk 的 alpha source 是法线贴图 Alpha 通道**（发丝透明度图；头发无 base 纹理，`BaseColorAlpha` 恒 1 会渲染成实心头盔——2026-09-12 实证修复，hair 与 charactertattoo 同走 `NormalAlpha`），且 hair 的 `alpha_mode=Blend` 按 **alpha clip** 处理：走 Cutout 管线，丢弃按 **4x4 screen-door 抖动**近似 alpha-to-coverage——固定阈值硬裁会把发丝边缘渐变带整片削碎（敖龙刘海实证），true blend 又会透出头皮鳞片。法线/光照路径不动。
- **皮肤族是介电质**（`family_params.z`，2026-09-12 实证修复）：skin/hair/iris/charactertattoo/characterocclusion 的 mask 通道是族别数据（皮肤 mask R=高光遮罩、G=光泽变化、B≈次表面强度、A=255；头发通道见上），通用回退把 `mask.b` 当金属度会让皮肤变成铜金属（敖龙 mask.b≈0.61 → 85% 金属实证）；皮肤族材质的金属度只取材质数据（恒 0），不再消费 mask.b。此前修正已及 `characterlegacy.shpk + Compatibility` 路径（无 ColorTable properties 时不把 mask G/B 误作 roughness/metalness，见 §通用材质）。
- **面妆 decal**（真实数据探测结论）：贴图 `chara/common/texture/decal_face/_decal_{id}.tex`，探测连续段 id 1..=69（xivModdingFramework `XivStrings.FacePaintFolder/File` 同规律；MTRL 无 decal sampler，游戏经 `Human->Decal` 运行态句柄绑定）。字节语义：bit7=reversed、bits0..6=面妆号；面妆号直连贴图号（维埃拉 24..29、61 验证），100..127 旧编号段补 `-100` 候选（103→3），128 经字节-100（→28）；当前游戏数据 32 组全覆盖，无候选命中时静默降级并保留 unsupported 摘要。贴图格式混合（BC4/BC1/B8G8R8A8），BC4 单通道解码为灰度 RGBA。FacePaintUVMultiplier/Offset 为运行态 cbuffer 值，离线按恒等 UV（uv0）处理。
- **种族骨变形烘焙**（回退网格的离线近似，2026-09-12 落地）：回退文件网格（维埃拉/硌狮小衣来自猫魅/鲁加文件、裸肤手足来自中原/拉拉男文件、共享发型来自中原文件）按**源族骨架 rest → 自身骨架 rest** 逐骨刚体替换烘焙（`skeleton::RaceDeform`：`deform = target_world × inverse(source_world)`，骨按名映射，目标缺骨时沿源父链取最近祖先的 deform；顶点按 blend 权重加权、法线 mat3 归一），修比例错位与手部扭曲（敖龙中原手实证）。游戏实际是 PBD 运行态骨变形，此为 TexTools 种族转换同级的骨架替换近似；身高/体格/胸围的 RGSP 缩放（human.cmp 尾部 32×140 字节记录）仍未解析、未应用。烘焙在装配加载内完成（源族骨架缺失静默跳过），渲染/蒙皮路径不变。
- **验证**：`tests/native_character_render.rs`（`XIV_GAME_DIR` + `--features "render-test-support,game-data"` + `--ignored`）：默认捏脸+调色板渲染中原男/猫魅女/敖龙女/维埃拉女（`installed-character-*.png`），目验小衣身体/皮肤/五官/面妆/手/足；全部件原位重叠。

渲染器侧配套修复：`create_material_bind_group` 原先按 draw batch 重复上传整套材质 GPU 纹理，多 submesh 模型（陆行鸟 demihuman 腿部单 MDL 拆出 96 个 batch）会放大到数十 GiB 触发 wgpu OOM；现在同一模型的全部批次共享按（源纹理索引 + 创建语义）去重的纹理缓存，渲染结果不变。

真实数据验证入口：`tests/native_model_domain_snapshots.rs`（`XIV_GAME_DIR` + `--features "render-test-support,game-data"` + `--ignored`），覆盖装备五部位/种族变体/饰品、家具与庭具、monster 宠物与坐骑、demihuman 陆行鸟及当前版本武器回归。

### 11.3 骨骼 rest pose 与 GPU 蒙皮

数据层 `crates/xiv-companion-data/src/skeleton.rs`（无 feature 门槛，纯数据 + 纯数学）+ 渲染层 `create_model_with_skeleton` 管线：

- **骨架加载**：sklb 容器头解析（magic 兼容 `blks`/`sklb`；版本 0021 u16 / 0030/0031 u32 偏移布局，未知版本在头 128 字节回退扫描 tagfile magic），Havok tagfile 经 vendored `physis_havok` 解出 `skeletons[0]`（bone 名/父索引/referencePose 局部 TRS）。vendored 解析器内部大量 panic，对外入口 `load_skeleton_from_sklb_bytes` 用 `catch_unwind` 包成 `SkeletonError::HavokPanic`（wasm 下 panic=abort，宁可解析失败不可炸页面），加载后再做长度/父索引/环/有限值校验。骨架**不走 serde/IndexedDB**（体积与版本问题）：`WeaponModelData` 不加 skeleton 字段，渲染 API 以参数传入，随加载内存存活。
- **路径模板**：角色 `chara/human/c{race:04}/skeleton/base/b0001/skl_c{race:04}b0001.sklb`（race code 来自 `customize.race_code()`）；monster/demihuman `chara/{monster|demihuman}/{m|d}{id:04}/skeleton/base/b0001/skl_{m|d}{id:04}b0001.sklb`。加载入口平行函数 `load_chara_model_with_skeleton_from_resource` / `load_character_assembly_with_skeleton_from_resource` 返回 `(Data, Option<ModelSkeleton>)`；sklb 缺失/解析失败 → `None` + 诊断日志，不报错（静态预览降级为非蒙皮）。原函数保留（内部调 with_skeleton 丢弃骨架）。
- **蒙皮数学**：`SkeletonPose` 为每骨局部 TRS 覆盖（`rest_pose()` 即默认 rest）；`world_matrices` 父链累乘（列主序对齐 wgpu）；`joint_matrices = world × inverse(bind world)`，inverse 由 `SkeletonInverseBindCache` 首次计算缓存；矩阵一律列主序 `[f32;16]`，四元数 xyzw。缺失 joint 名回退单位阵。
- **渲染接线**：`ModelRenderContext::create_model_with_skeleton(model, options, Option<&ModelSkeleton>)`。实例 joint 表 = 全部渲染 mesh `bone_table` 名并集（首见顺序，跨 mesh/部件按名合并，≤256 超出截断记日志）；顶点 blend 索引是 **bone_table 绝对下标**（submesh 的 `bone_start_index`/`bone_count` 只是该 submesh 引用窗口的声明，不参与重映射——d0001e0001_dwn 实测：窗口偏移解释把 submesh1+ 网格完全撕碎（边长拉伸 100%），绝对下标边长守恒；rest 蒙皮逐像素等价因此 bug 在 rest 下不可见）。joint 矩阵在实例级 group(2) storage buffer（头 u32 joint 数 + 256×64B，STORAGE|COPY_DST 预分配）；`ModelInstance::update_joint_matrices(&[[f32;16]])` 增量写；`update_materials` 只重建 group(1)，joint 绑定不受影响。`ModelRenderer` 兼容封装同步加 `*_with_skeleton` 入口。
- **GPU 顶点布局**：GpuVertex 尾部加 8 蒙皮槽——8 个 u8 joint 索引**打包成一个 `Uint32x2`**（location 13，低字节在前），weights0/weights1 两个 Float32x4（location 14/15）。原计划 joints0/joints1 各占一个 location 会到 16，超出设备 limits 的 `max_vertex_attributes = 16`（WebGL2 互操作只保证 16），打包后共 16 个顶点属性、布局无填充（204B）。无蒙皮数据顶点写 joints=0/weights0=(1,0,0,0)。
- **WGSL**：`JointData` storage（头 joint 数 + mat4 运行时数组）；joint 数 0 走原 vs_main/vs_outline 路径（渲染与非蒙皮逐位一致）；蒙皮 `skinned_pos = Σ w·J·pos`，权重是 u8/255 量化值，GPU 按 Σw 归一兜底；法线用 mat3(J)——刚体骨骼精确，非均匀缩放骨骼近似（注释声明）。
- **identity 一致性**：rest pose 关节矩阵为恒等，实测（640×640）与非蒙皮输出差异 ≤ 数十像素、±1 LSB 为主（f16 量化边界），个别刀锋像素因 ~1e-7 位置扰动跨纹理边界换采相邻纹素——多骨顶点浮点求和数学上无法逐位相等；任何真实链路错误都是万级像素差。39 合成快照走 joint 数 0 旧分支零扰动。
- **web 端前置**：storage buffer 需要 device limits `max_storage_buffers_per_shader_stage ≥ 1` 且 `max_storage_buffer_binding_size ≥ 16400`；web 画布 limits 基线 `downlevel_webgl2_defaults().using_resolution(adapter.limits())` 的 storage 相关项全 0，在此基础上补这两个下限即可（WebGPU 后端实际能力均满足），`ModelRenderContext::new` 校验不再失败。
- **验证**：`tests/native_skeleton_smoke.rs`（`--ignored`）：陆行鸟（86 bones，99/99 mesh 带 bone table）+ 中原男装配（106 bones，27/27）rest 蒙皮与非蒙皮等价；陆行鸟 `j_kubi_a`（颈根，链 `j_kubi_a→b→c→j_kao`）绕 X 转 30° 头部明显偏移。数据层 16 个矩阵单测（恒等/两骨链手算/子集回退/inverse 精确性/路径/panic 包装）。

### 11.4 PAP 骨骼动画：解析、采样与前端播放

数据层 `crates/xiv-companion-data/src/animation.rs`（无 feature 门槛，纯数据 + vendored havok）+ 页面 rAF 播放接线：

- **pap 容器**：固定头 26 字节（magic `pap `、num_animations@8、havok_position@18、tmb_offset@22）+ 名表 40 字节/条（name[32]/type u16/havok_index i16/face i32）；内嵌 havok = `[havok_position, tmb_offset)`（tmb_offset 异常取到文件尾），tagfile 根对象 `hkaAnimationContainer`。
- **animations/bindings 关联**（对安装版真实数据验证）：container 的 `animations` 与 `bindings` 严格平行等长，名表 `havok_index` 即数组下标；vendored `HavokAnimationContainer` 只保留 `bindings`（binding 内含装箱 animation），`bindings[havok_index]` 即该动画。`binding.transform_track_to_bone_indices` 把 transform track 映射到**本 skeleton** 的 bone 下标（越界 track 丢弃 + 计数日志，不 panic），`number_of_transform_tracks == binding 表长`。FFXIV 实测只有 `hkaSplineCompressedAnimation`，量化仅 POLAR32/THREECOMP40/THREECOMP48 + BITS8/16——采样器对 THREECOMP24/STRAIGHT16/UNCOMPRESSED 与未知值直接 panic，加载时用 `quantization_histogram` 预检，含不支持量化的 animation 整条剔除（加载即保证采样路径不 panic，wasm panic=abort 下不可依赖 catch_unwind）。
- **采样**：vendored `sample(time_ms)` 入参**毫秒**（内部除 1000 转秒），输出每 track 局部 TRS；`sample_animation_pose(set, index, time_ms, skeleton)` 落到 `SkeletonPose`（rest 起步、未覆盖骨骼保持 rest，时间钳制 `[0, duration_ms]`），`animation_joint_matrices` 直出关节矩阵（共享 `SkeletonInverseBindCache`）。`duration_ms = duration() × 1000`。**vendored 修复**（2026-09-12，本轮动画联调实证）：`read_knots` 的局部 knot 窗口原按 `raw[i+1] + span - p` 重建，仅在 span==p 首段正确（t=0 恒为首控制点所以静态目验无恙），span 走深后 evaluate 权重爆掉、四元数分量冲到 12（姿态整只翻转）；已改为标准 Piegl A3.1 窗口 `raw[span-p+1+i]`，evaluate 递推不变。分量式四元数 NURBS 插值在控制点符号翻转处会插值出 ~0.3 的模长（渲染端 trs_to_mat4 归一化吸收），属已知限制。
- **路径探测**（`pap_path_candidates`）：角色 `chara/human/c{race}/animation/a0001/bt_common/resident/action.pap` + 常用 emote 名（joy/wave/beckon/dance/salute/point/laugh/doze）逐个探测；monster/demihuman `{cat}/{m|d}{id}/animation/a0001/bt_common/resident/mount.pap` + `idle.pap`。多候选逐个 file_exists，命中的全部解析合并（`ModelAnimationSet::append`），全缺返回空集不报错——动画是 best-effort 增强，不动模型加载成败。
- **加载入口**：本地 BrowserSqPack 版 `load_chara_model_with_skeleton_from_local` / `load_character_assembly_with_skeleton_from_local`（`src/app/resources.rs`）在模型 + sklb 之后顺手探测 pap（数据层 `load_animation_set_from_async_resource`），进度上报附动画数。同步 Resource 的 with_skeleton 版本此前已落地，本轮补齐 async 平行版。
- **web 播放数据流**：页面持 `Rc<ModelAnimationSet>` + `Rc<ModelSkeleton>`，右侧栏有动画集时出现「动画」下拉（None=rest + 各动画名，选择按物品 id 作用域存储）；选中项 + 资源指针打包成 `AnimationPlaybackState` 传入画布，镜像进 Signal 供 rAF 循环每帧读取；循环内 `time = (rAF_now - start) % duration_ms`（切换动画/模型时重置计时起点），`sample_animation_pose → SkeletonInverseBindCache::joint_matrices → WebModelCanvasRenderer::update_joint_matrices` 增量上传；切回 Rest 上传一次 rest 关节矩阵；实例重建（set_model）经 `joint_epoch` 信号刷新 joint 名表缓存。采样与矩阵计算在 UI 线程每帧一次（~200 bone 全链 + ≤256 mat4），实测远低于帧预算。**平铺预览与动画互斥**：component 偏移量烘在蒙皮前顶点位置里（rest 下逐像素等价），动画驱动 joint 后偏移随关节旋转；有动画集时画布强制原位布局（`component_preview_layout=false`），无动画集保持原布局。
- **prepared requirements**：动画不新增 requirement 位——`skeleton_pose`/`skinning_matrices` 已覆盖蒙皮负载声明；采样是纯运行时行为（wall clock 驱动），`PreparedModelOptions` 无动画字段；唯一运行时是 device limits 的 storage buffer 下限（见 §11.3）。
- **降级**：无 skeleton / pap 全缺 / 全部解析失败 → 无动画集 → 下拉不出现、渲染与现状完全一致（joint 数 0 走非蒙皮路径；有 skeleton 无动画则 joint 缓冲保持 rest）。
- **验证**：`tests/native_animation_smoke.rs`（`--ignored`）：陆行鸟 d0001 + mount.pap（t0 骑乘站姿与 bind 差 2.8 万像素、t40 与 t0 差 3.1 万、循环收尾 max quat delta 0.016、全程 |q| 有界）、古菩 m0054 同测、中原男 + action.pap 单帧 t=50%（头在骨盆上方 + 关节矩阵有限性）；数据层单测覆盖垃圾字节/坏头/缺 tagfile（→Err 不 panic）、时间钳制回退、路径模板、mock 资源多候选探测（缺失/垃圾容忍）。遗留：emote 探测名是少量已知名枚举（pap 无目录可列）；TMB 时间线（动画衔接/音效）未解析；动画混合（blend hint）只取 Normal 语义、不做叠加；mount/idle 循环的起始帧是骑乘站姿而非 bind（与任务预期的"起始帧≈bind"不同，实测数据如此）。
