...
7. 染色：Legacy/Dawntrail `ColorDyeTable` 的 template、channel 和可染通道 flag 已结构化进 `ModelMaterial.colorDyeTable`，并与兼容字段 `hasColorDyeTable` 一起驱动 `usesDye` 和 prepared unsupported summary；下一步接入 `chara/base_material/stainingtemplate.stm`、EXD stain 参数和用户 stain 输入，真正应用到 ColorTable 或 renderer override。
