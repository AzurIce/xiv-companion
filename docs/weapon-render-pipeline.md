...
`lightshaft.shpk` 的 `g_Color`、`g_TexAnim`、`g_TexU`、`g_TexV`、`g_Ray` 已解析进材质数据和 renderer uniform。`g_Sampler0/1` 会分别进入 base/secondary-base sRGB binding；LightShaft draw role 以 vertex color B 计算 `mix(Sampler0, Sampler0 * Sampler1)`，结果乘 `g_Color`，颜色 scalar 作为 emission strength，并与 vertex alpha 相乘得到 additive alpha。MeddleTools 节点图未连接 `g_TexAnim/U/V/Ray`，因此这些值不再产生推测的 UV/强度效果，只保留用于审计。`Type/AngleClip/NearClip` 等行为仍未实现。
