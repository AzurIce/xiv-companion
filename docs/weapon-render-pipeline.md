...
4. Material params：alpha/glass/normal/tile/toon/detail/color/outline/scroll/lightshaft 参数、`CategoryFlowMapType`、`GetValues` 与 `GetDecalColor` 已结构化；water colors/wave 和 bguvscroll Map1 sampler roles 也已加入。共享 tile/detail atlas、Map0/Map1 scroll、Flow tangent 与 water alpha/base/primary normal 已有 WGSL 消费。后续处理 multi map mask、runtime skin decal 输入、water refraction/whitecap、detail influence 和其它 shader-family 行为。
