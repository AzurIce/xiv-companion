...
4. 渲染器：characterTransparency/glass、dither depth、GlassBlend、outline、toon、bguvscroll Map0/Map1、Flow、stockings opaque alpha/pipeline、tattoo normal-A alpha 与 water direct alpha/deep color/primary wave 已完成第一版。tattoo 的 OptionColor/DecalColor 混色仍不猜测。secondary color/normal/specular 只在 `BgUvScroll + GetMultiValues` 中复用 tile/sheen/sphere 物理 binding，按 UV1Scroll 和 vertex alpha 混合；其它 `GetValues` 变体保持显式未支持，sampled texture 数仍为 15。当前 secondary normal 虽已上传 `normal1/bitangent1`，WGSL vertex output 却丢弃它们并错误复用 primary tangent frame；Meddle 按 usage index 成组保留 `Normals[]/Binormals[]`。本轮将只为 `BgUvScroll + GetMultiValues` 的 Map1 normal 使用 secondary frame，缺失通道沿用既有 CPU primary fallback，并以 synthetic WGPU fixture 固定两套相反 frame 验证输出差异；`color1/flow1` 继续等待 family-specific 证据。characterReflection generic approximation 与 stockings/tattoo/occlusion runtime 输入均已有独立 diagnostic；后续再寻找真实 reflection 节点/样本。
