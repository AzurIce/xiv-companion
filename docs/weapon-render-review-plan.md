...
5. 验证：45052 第二通道/metallic 染色 case 已补齐，并与 baseline/stain0 做 application row count 和像素差异断言；weapon scope 已确认无 bg detail 样本，相关权重等待游戏 shader 或非武器 fixture。继续为 transparency/glass/scroll 等行为增加 synthetic 与真实 snapshot，并用 catalog audit 防止真实加载缺口被小型 phantom 集合漏掉。
