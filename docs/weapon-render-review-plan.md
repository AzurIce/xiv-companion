...
- `g_ToonIndex`、`g_ToonLightScale`、`g_SheenRate`、`g_SheenTintRate`、`g_SheenAperture`、`g_SphereMapIndex` 已按 Meddle `Names.cs` CRC/default 结构化进 `ModelMaterial`、phantom summary 与 renderer `toonSheenParams` / `sheenSphereParams` uniform；WGSL 已把 sheen/sphere 常量作为 ColorTable extra ramp 之外的保守高光/反射输入。调查确认还缺 `g_ToonLightSpecAperture`、`g_ToonReflectionScale`、`g_ToonSpecIndex` 三项已知参数；MeddleTools 源码和 Blender character node group 均无 toon socket/node，24 组 phantom 现有 toon 参数均为默认值。下一批先补齐完整参数族，再做不依赖伪造 lookup texture 的解析式 character-family toon 近似。
...
- 已完成：`g_ToonIndex`、`g_ToonLightScale`、`g_SheenRate`、`g_SheenTintRate`、`g_SheenAperture`、`g_SphereMapIndex` 进入 `ModelMaterial`，默认值分别为 `0`、`2`、`0`、`0`、`1`、`0`；renderer uniform 已进入 WGSL，其中 `g_SheenRate` / `g_SheenTintRate` / `g_SheenAperture` 会补充 sheen 高光，`g_SphereMapIndex` 会影响已有 sphere-like rim tint。下一步结构化 `g_ToonLightSpecAperture=50`、`g_ToonReflectionScale=2.5`、`g_ToonSpecIndex≈0`，增加 prepared `usesToon`，并在 WGSL 消费完整参数族。
...
3. 渲染器：characterTransparency/glass 的 normal-B alpha、lighting policy、`DrawDepthMode_Dither` depth-only prepass、显式 `GlassBlendMode` scene option 与 character-family inverted-hull outline pass 已完成第一版。45059 验证透明/玻璃路径，45052 的 synthetic outline override 验证独立轮廓几何；真实 outline width 换算仍缺样本。下一步补齐 toon 参数族并实现 prepared/WGSL toon lighting，通过默认与 synthetic override snapshot 控制视觉变化。之后推进 shader-family-specific scroll/flow。water/river 的 `g_Transparency` 单独推进，tile ORB/detail 权重在获得真实样本后再校准。
