...
3. 已完成 WeaponCatalog 级 shader-family 扫描：修正 equipment-style 拳套后，8281 个武器条目、7365 个唯一模型、8110 个可解析材质中没有 bg/bguvscroll；因此 weapon preview 内无真实 bg detail 样本，MeddleTools 最终 influence 仍标为 borked，在取得游戏 shader 或非武器 fixture 前不校准猜测权重。23 个 equipment-style 拳套已恢复，剩余 4 个材质异常转入下一批路径/解析修正。
...
1. 数据解析：WeaponCatalog shader-family audit 已确认当前武器材质为 character/skin/characterGlass；23 个 equipment-style 拳套已通过默认 c0101 glove + human body material fallback 恢复，并保留 race/skeleton/SkinColor 缺失诊断。当前调查 `w3054 -> mt_w3103` 的 root/version 实际位置，以及 `w0371/v0201/mt_w0321b0033_a.mtrl` 的 raw header 是否为占位/新格式；修正后再继续 shader-family-specific texture role/UV 规则。runtime GPU ColorTable 继续只作为 unsupported 输入标记。
