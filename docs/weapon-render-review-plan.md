...
- WeaponCatalog shader-family audit 同时暴露 27 个静态加载缺口：23 个拳套条目的 ModelMain 是 equipment-style model、当前 `chara/weapon` candidate 无法读取；另有 2 个跨 weapon-root 材质候选缺失和 2 个 MTRL 解析失败。报告写入 `target/weapon-shader-family-audit.json`，并支持 `XIV_WEAPON_SHADER_SCAN_LIMIT` 做快速子集扫描。下一步优先调查并修正 equipment-style 拳套路径/角色依赖，再分别处理剩余材质异常。
...
1. 数据解析：WeaponCatalog shader-family audit 已确认当前武器材质只有 character/characterGlass，并暴露 23 个 equipment-style 拳套模型、2 个材质路径缺失和 2 个 MTRL 解析失败。下一步先修 equipment-style 拳套的路径/角色依赖，再处理剩余材质异常；之后继续 shader-family-specific texture role/UV 规则。runtime GPU ColorTable 继续只作为 unsupported 输入标记。
