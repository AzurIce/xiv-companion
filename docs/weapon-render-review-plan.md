...
- `lightshaft.shpk` 的 `g_Color`、`g_TexAnim`、`g_TexU`、`g_TexV`、`g_Ray` 已结构化进 `ModelMaterial` 和 phantom summary；当前 renderer 会对 `LightShaft` draw role 启用 additive pass，但只保留第一张 `g_Sampler0`，并为 `g_TexAnim/U/V/Ray` 添加了无节点证据的 UV/强度近似。Blender headless 检查 MeddleTools `meddle lightshaft.shpk` 已确认实际可复刻部分：`g_Sampler0` 与 `g_Sampler1` 均为 sRGB/Linear/Repeat，先用 vertex color B 驱动 Multiply blend，再整体乘 `g_Color`；alpha 为该颜色转 scalar 后乘 vertex alpha。`g_TexAnim/U/V/Ray` sockets 在节点图中均未连接。本轮将保留第二张纹理并按这些连接修正 WGSL，同时删除未连接参数对输出的推测影响；原始参数仍保留用于审计。
...
4. 渲染器：characterTransparency/glass、dither depth、GlassBlend、outline、toon、bguvscroll Map0/Map1、Flow、stockings opaque alpha/pipeline、tattoo normal-A alpha 与 water direct alpha/deep color/primary wave 已完成第一版。tattoo 的 OptionColor/DecalColor 混色仍不猜测。secondary color/normal/specular 只在 `BgUvScroll + GetMultiValues` 中复用 tile/sheen/sphere 物理 binding，按 UV1Scroll 和 vertex alpha 混合；其它 `GetValues` 变体保持显式未支持，sampled texture 数仍为 15。Map1 normal 现已使用 secondary tangent frame：WGSL vertex output 会传递 `normal1/bitangent1`，fragment 分别把 Map0/Map1 normal 转到各自世界空间后再按 vertex alpha 混合；缺失通道沿用既有 CPU primary fallback。synthetic WGPU fixture 固定旋转后的第二套 frame，确认最终像素差异超过阈值。Meddle 按 usage index 成组保留 `Normals[]/Binormals[]`；`color1/flow1` 继续等待 family-specific 证据。本轮优先修正 lightshaft：把 `g_Sampler1` 独立路由到已有 secondary base binding，以 vertex color B 复刻 Blender Multiply blend，结果乘 `g_Color`，并按颜色 scalar × vertex alpha 输出 additive alpha；删除 `g_TexAnim/U/V/Ray` 的无证据视觉近似。characterReflection generic approximation 与 stockings/tattoo/occlusion runtime 输入均已有独立 diagnostic；后续再寻找真实 reflection 节点/样本。
