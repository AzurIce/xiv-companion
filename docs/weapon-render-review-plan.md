...
- `crystal.shpk` 当前仍落入 `MaterialShaderFamily::Unknown`，导致 Environment 已解析但未渲染的事实没有明确 family/unsupported 语义。Blender headless 检查 MeddleTools `meddle crystal.shpk`：接口包含 `g_SamplerEnvMap`，但该输入在现有节点图中没有任何连线，不能据此实现可信混合。下一批新增 `Crystal` family、`usesEnvironmentMap` feature flag 和 `environmentMapping` unsupported 标记；renderer 在获得真实节点/样本证据前继续不采样 Environment。
...
2. 结果处理：当前先让 Crystal/Environment 的“已解析、未渲染”状态进入明确 prepared family/feature/unsupported 字段；随后把当前在 WGSL 中的 tile layer、detail layer、ORB 通道和组合权重逐步提升为更明确的 prepared 规则，尤其处理 TileMatrix float channels、detail/multi-detail mask 和越界诊断，减少 renderer 内部猜测。
