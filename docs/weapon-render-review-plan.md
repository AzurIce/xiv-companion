...
1. 数据解析：WeaponCatalog shader-family audit 已确认当前武器材质为 character/skin/characterGlass；23 个 equipment-style 拳套已通过默认 c0101 glove + human body material fallback 恢复，并保留 race/skeleton/SkinColor 缺失诊断。下一步处理 2 个跨 weapon-root 材质候选缺失和 2 个 MTRL 解析失败，再继续 shader-family-specific texture role/UV 规则。runtime GPU ColorTable 继续只作为 unsupported 输入标记。
