...
最近核验：2026-07-13；最近实现基线：`f2f897d`。
...
- 15 个现有 texture bindings 使用逐 role sampler；base/emissive、data maps、index/arrays/extra ramps 不再共用错误采样策略。
...
- 独立 sampler policy、ColorTable extra maps、共享 tile/detail atlas 和 float/HDR payload。
