...
- `PreparedMaterialUnsupportedInputs` 已有第一版，按当前可可靠判断的数据标出 dye application、runtime ColorTable、decal/crest、runtime material change、tile array、detail array、secondary map blend、incomplete shader family logic，phantom summary 会随 prepared material 输出这些缺口。下一批将 shader-family runtime 输入进一步拆成 `runtimeOptionColor`、`runtimeDecalColor` 与 `runtimeSkinMaterial`：MeddleTools `charactertattoo` 节点证明 tattoo 颜色依赖 OptionColor/DecalColor，Meddle composer/parse util 证明二者来自 customize/decal constant buffer；Meddle on-render util 证明 stockings 会复制 runtime skin material textures。
...
1. 数据解析：共享 arrays 与 runtime fallback 本轮已贯通；当前先细化 stockings/tattoo 的 runtime input diagnostics，避免把缺少 OptionColor、DecalColor、skin material texture 的材质只归为笼统 family 缺口；随后继续结构化 shader-family-specific texture role/UV 规则，并寻找能覆盖 bg detail、reflection、stockings、tattoo、occlusion 的真实武器样本。runtime GPU ColorTable 继续只作为 unsupported 输入标记。
