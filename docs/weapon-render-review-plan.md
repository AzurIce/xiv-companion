...
- `g_DetailID`、`g_MultiDetailID`、`g_DetailColor`、`g_MultiDetailColor`、`g_DetailColorUvScale`、`g_DetailNormalUvScale` 已结构化进 `ModelMaterial` 和 renderer detail uniforms；WGSL 会按两个 detail ID 选层，用 color/normal 各自 UV scale 采样 detail pair atlas，并以 0.5 为 diffuse 中性值做保守 tint/normal 组合。Blender 节点检查确认 `bg_detail_blend.MultiBlendWeight` 同时驱动 primary/multi diffuse 与 normal 的 A/B mix；renderer 现已在 bg `GetMultiValues` 时用 vertex alpha 统一 mix 两层，Single 固定 primary，数组缺失时的 procedural tint fallback 也使用同一规则。detail 对 base 的最终 influence reroute 在 MeddleTools 中仍未连接且源码标为 `currently borked`，因此没有可采信的最终权重公式。新增 ignored WeaponCatalog shader-family audit 会轻量读取 MDL metadata/MTRL，不解码几何和纹理；本地完整扫描 8281 个武器条目、7365 个唯一模型、8071 个可解析材质，结果为 8065 `character`、6 `characterGlass`、0 `bg/bguvscroll`。因此武器预览范围内不存在真实 bg detail 校准样本，在获得游戏 shader 或非武器 bg fixture 前不继续猜最终 influence。
- WeaponCatalog shader-family audit 同时暴露 27 个静态加载缺口：23 个拳套条目的 ModelMain 是 equipment-style model、当前 `chara/weapon` candidate 无法读取；另有 2 个跨 weapon-root 材质候选缺失和 2 个 MTRL 解析失败。代表值 `0x0000000000012276` 不应继续解释为 weapon `w8822/b0001`，而应按 equipment set `0x2276=8822`、variant `1` 调查。Meddle 对 Human 模型通过 `EquipmentModelId + HumanEquipmentSlotIndex + ResolveMdlPath` 解析实际 race-specific MDL，并从模型 slot 取 stain/runtime material；离线预览不能仅增加一个伪 `chara/weapon` fallback。本轮计划先验证默认人族 hand/glove MDL 与 material 路径，结构化 equipment-style candidate，再明确缺少角色种族/骨架输入时的 fallback 与诊断。报告写入 `target/weapon-shader-family-audit.json`，并支持 `XIV_WEAPON_SHADER_SCAN_LIMIT` 做快速子集扫描。
...
3. 已完成 WeaponCatalog 级 shader-family 扫描：8281 个武器条目、7365 个唯一模型、8071 个可解析材质中没有 bg/bguvscroll；因此 weapon preview 内无真实 bg detail 样本，MeddleTools 最终 influence 仍标为 borked，在取得游戏 shader 或非武器 fixture 前不校准猜测权重。扫描暴露的 23 个 equipment-style 拳套模型和 4 个材质异常转入下一批路径/解析修正。
...
1. 数据解析：WeaponCatalog shader-family audit 已确认当前武器材质只有 character/characterGlass，并暴露 23 个 equipment-style 拳套模型、2 个材质路径缺失和 2 个 MTRL 解析失败。当前优先把 `0x0001_2276` 一类值按 equipment set/variant 而非 weapon model/body 解码，验证默认人族 glove 路径、材质根和 mesh 可用性，并为 race/skeleton 缺失提供显式 fallback/diagnostic；再处理剩余材质异常。runtime GPU ColorTable 继续只作为 unsupported 输入标记。
