...
- `characterreflection.shpk` 已有 family 分类，但当前没有独立 prepared 缺口：Meddle/MeddleTools 搜索未发现静态 reflection sampler、on-render replacement 或对应节点组，MeddleTools 也没有把它映射回 `character.shpk`。现 renderer 因而只是 generic character approximation，不能把 Environment/sphere/specular 资源猜作 reflection 输入。下一批增加独立 `characterReflection` unsupported 字段，并保留现有 `incompleteShaderFamilyLogic`，直到获得真实 SHPK 节点或样本证据。
...
3. 渲染器：characterTransparency/glass、dither depth、GlassBlend、outline、toon、bguvscroll Map0/Map1、Flow、stockings opaque alpha/pipeline 与 water direct alpha/deep color/primary wave 已完成第一版。secondary color/normal/specular 只在 `BgUvScroll + GetMultiValues` 中复用 tile/sheen/sphere 物理 binding，按 UV1Scroll 和 vertex alpha 混合；其它 `GetValues` 变体保持显式未支持，sampled texture 数仍为 15。当前先让 characterReflection 的 generic approximation 进入独立 unsupported diagnostic；stockings/tattoo/occlusion runtime 输入继续显式标记，后续再寻找真实 reflection 节点/样本。
