...
- `g_SamplerEnvMap` 当前仍被压入 `Other`，会丢失 BG/crystal 环境贴图的独立身份。MeddleTools texture config 明确其为 Non-Color、Linear、Repeat；Meddle Names 则表明 `g_EnvMapPower` 属于 bg/bgcolorchange/bgcrestchange/bgprop/bguvscroll/crystal，而不是 `characterreflection.shpk`。下一批新增独立 `Environment` texture kind、同步/异步 loader slot、model/prepared binding/sampling 和 Web diagnostics；在缺少可靠反射坐标与混合节点证据时不接入 WGSL，也不把它误当 character reflection。
...
1. 数据解析：共享 arrays 与 runtime fallback 本轮已贯通；stockings/tattoo runtime input diagnostics 已拆分。当前推进 `g_SamplerEnvMap` 独立 Environment role，避免继续丢进 `Other`；随后继续结构化 shader-family-specific texture role/UV 规则，并寻找能覆盖 bg detail、reflection、stockings、tattoo、occlusion 的真实武器样本。runtime GPU ColorTable 继续只作为 unsupported 输入标记。
