...
- `GetMaterialValue` 当前只进入 material debug 的 known-name 白名单，尚未结构化进材质/prepared。Meddle `Names.cs` 明确 `skin.shpk` key `0x380CAED0`、默认 Face，以及 Body/BodyJJM/FaceEmissive 四种 value；MeddleTools 只对 `skin.shpk + Face` 的 diffuse 设置 Linear/EXTEND，以避免越界 UV wrap 出牙齿，Body mapping 仍标为 TODO。本轮新增 `MaterialSkinValueMode`，同步/异步 composition 与 phantom summary 保留四种模式；仅 Face 将 prepared base-color address mode 改为 ClampToEdge，其它 texture role 与 Body variants 继续 Repeat。49100 安装数据回归用于确认 weapon scope glove skin 的实际模式，避免把 face 规则泛化到所有 skin。
