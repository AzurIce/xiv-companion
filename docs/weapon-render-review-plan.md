...
- `GetDecalColor / 0xD2777173` 目前仅在 raw material summary 中可见。Meddle 明确 Off/Alpha/RGBA values；MeddleTools skin 只映射 Alpha，并同时连接 runtime `DecalColor`、strength 与 `DecalTexture`，tattoo 则无条件连接 runtime `g_DecalColor`。Meddle composer 证明颜色来自 customize/decal constant buffer，on-render output 证明纹理是运行时输入。本轮将结构化 `MaterialDecalColorMode::{Off,Alpha,Rgba,Unknown}` 与 raw value，贯通同步/异步 loader、prepared/phantom summary；skin Alpha/Unknown 增加 runtime decal color/texture 与未实现 mode diagnostics，tattoo 现有无条件 runtime decal color 保持不变，不猜混色公式。
