...
- `GetSubColor` 当前只存在于 material debug known-name 表，没有进入 `ModelMaterial`/`PreparedMaterial`。Meddle Names 明确 key `0x24826489` 与 Face/Hair values，MeddleTools hair mapping 也消费两种模式；`GetSubColorFace` 同时适用于 characterocclusion/charactertattoo，实际颜色来自 Meddle composer 注入的 customize buffers。下一批新增 `MaterialSubColorMode::{None,Face,Hair,Unknown}`，贯通同步/异步 composition，并增加 `runtimeSubColor` unsupported：characterOcclusion 或显式 Face/Hair 模式都标出缺失运行时颜色，不在离线 loader 中伪造。
...
1. 数据解析：共享 arrays 与 runtime fallback 本轮已贯通；stockings/tattoo runtime input diagnostics 已拆分，`g_SamplerEnvMap` 也已成为独立 Environment role。当前结构化 `GetSubColor` Face/Hair mode，并把 occlusion 的 runtime sub-color 依赖进入 prepared diagnostics；随后继续 shader-family-specific texture role/UV 规则和真实样本调查。runtime GPU ColorTable 继续只作为 unsupported 输入标记。
