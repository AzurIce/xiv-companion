...
- renderer 已有第一版 debug view，能切换 base、normal、mask/material、specular、emissive、alpha、UV、vertex color、mesh/draw-role color、ColorTable index、material map、multi map 与 ColorTable extra maps；更细的 per-role sampler policy、真实 tile/detail array 诊断仍未实现。
...
4. 继续补 debug render modes：base、normal、mask/material、specular、emissive、alpha、UV set、vertex color、mesh/draw-role color、ColorTable index、material map、multi map、ColorTable extra maps 已有第一版；后续补 per-role sampler policy 和真实 tile/detail array 诊断，作为真实武器样本回归的主要判断工具。
...
- renderer 已用 `Rgba8Unorm` 创建 normal/mask/material-properties，并用 data sampler 采样；ColorTable extra maps 已用 nearest sampler 采样并提供 debug view。仍需 synthetic shader fixture 验证 tile/sheen/sphere 可见效果。
