...
pub fn InventoryPage() -> Element {
    ...
    rsx! {
        div { class: "flex h-[calc(100dvh-3.5rem)] min-w-0 flex-col overflow-hidden bg-background lg:h-screen",
            div { class: "flex flex-wrap items-center justify-between gap-2 border-b px-4 py-2 sm:px-5 lg:px-6",
                div { class: "min-w-0 space-y-0.5",
                    div { class: "text-xs text-muted-foreground", "本地角色数据" }
                    div { class: "flex flex-wrap items-center gap-x-2 gap-y-1",
                        h1 { class: "text-xl font-semibold leading-tight", "物品" }
