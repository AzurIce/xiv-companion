...
use crate::vendor::physis_havok::byte_reader::ByteReader;
...
impl HavokSplineCompressedAnimation {
    ...
    // NOTE(vendored): read-only accessors added for validation/statistics; parsing and
    // sampling logic below is unchanged from upstream.
    pub fn number_of_transform_tracks(&self) -> usize {
        self.number_of_transform_tracks
    }
    ...
    pub fn max_frames_per_block(&self) -> usize {
    ...
    /// Count raw quantization values across every block/track mask header, mirroring how
    /// `sample` walks the mask stream. Returns `(rotation[16], translation[4], scale[4])`
    /// indexed by the raw packed value (see `unpack_quantization_types`); slot meanings:
    /// rotation 0..=5 = POLAR32/THREECOMP40/THREECOMP48/THREECOMP24/STRAIGHT16/UNCOMPRESSED,
    /// translation/scale 0/1 = BITS8/BITS16.
    pub fn quantization_histogram(&self) -> ([u32; 16], [u32; 4], [u32; 4]) {
        let mut rotation_counts = [0u32; 16];
        let mut translation_counts = [0u32; 4];
        let mut scale_counts = [0u32; 4];
        ...
        for block in 0..self.num_blocks {
            ...
            for _ in 0..self.number_of_transform_tracks {
                ...
                translation_counts[(packed_quantization_types & 0x03) as usize] += 1;
                rotation_counts[((packed_quantization_types >> 2) & 0x0f) as usize] += 1;
                scale_counts[((packed_quantization_types >> 6) & 0x03) as usize] += 1;
                // per-track translation/rotation/scale masks
                mask.read();
                mask.read();
                mask.read();
            }
        ...
        (rotation_counts, translation_counts, scale_counts)
    }
    ...
    fn read_knots(
        ...
        for i in 0..2 * p {
            // NOTE(vendored): upstream reconstructed the local knot window as
            // `raw[i + 1] + span - p`, which only equals the true window
            // `raw[span - p + 1 + i]` when span == p — i.e. correct only at the
            // first span (so t=0 always sampled fine and later spans returned
            // garbage, e.g. quaternion components of magnitude 12). Fixed to the
            // standard Piegl & Tiller A3.1 window starting at span-p+1; the
            // `evaluate` recurrence below is unchanged and matches A3.1 with
            // this window (verified against d0001 mount.pap: j_kosi quaternion
            // stays unit-length and the render stays coherent at all times).
            let item = raw[span + 1 - p + i] as usize;
